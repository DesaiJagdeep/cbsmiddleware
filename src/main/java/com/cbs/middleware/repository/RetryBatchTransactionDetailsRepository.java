...
public interface RetryBatchTransactionDetailsRepository extends JpaRepository<RetryBatchTransactionDetails, Long>, JpaSpecificationExecutor<RetryBatchTransactionDetails> {
    ...
    @Query(value = "select count(id) from retry_batch_transaction_details where retry_batch_transaction_id In(select id from retry_batch_transaction where batch_ack_id=:batchAckId)", nativeQuery = true)
