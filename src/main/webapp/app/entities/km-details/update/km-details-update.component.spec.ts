...
describe('KmDetails Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call kmMaster query and add missing value', () => {
      ...
      const kmMaster: IKmMaster = { id: 264 };
      kmDetails.kmMaster = kmMaster;

      const kmMasterCollection: IKmMaster[] = [{ id: 1075 }];
      jest.spyOn(kmMasterService, 'query').mockReturnValue(of(new HttpResponse({ body: kmMasterCollection })));
      const expectedCollection: IKmMaster[] = [kmMaster, ...kmMasterCollection];
      ...
      expect(kmMasterService.addKmMasterToCollectionIfMissing).toHaveBeenCalledWith(kmMasterCollection, kmMaster);
      expect(comp.kmMastersCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const kmDetails: IKmDetails = { id: 456 };
      const kmMaster: IKmMaster = { id: 13997 };
      kmDetails.kmMaster = kmMaster;

      activatedRoute.data = of({ kmDetails });
      comp.ngOnInit();

      expect(comp.kmMastersCollection).toContain(kmMaster);
