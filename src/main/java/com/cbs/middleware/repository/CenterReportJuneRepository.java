...
public interface CenterReportJuneRepository extends JpaRepository<CenterReportJune, Long>, JpaSpecificationExecutor<CenterReportJune> {
    ...
    @Query(value = "select count(distinct aadhar_number),sum(interest_second_15),sum(interest_second_25),upto_50000,social_category  from center_report_june where pacs_number=:pacsNumber and financial_year=:financialYear group by upto_50000,social_category", nativeQuery = true)
