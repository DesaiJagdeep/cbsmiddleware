...
public interface RetryBatchTransactionDetailsRepository extends JpaRepository<RetryBatchTransactionDetails, Long>, JpaSpecificationExecutor<RetryBatchTransactionDetails> {
    ...
    @Query(value = "select count(*) from retry_batch_transaction_details where status=1 And retry_batch_transaction_id In(select id from retry_batch_transaction where batch_ack_id=:batchAckId)", nativeQuery = true)
