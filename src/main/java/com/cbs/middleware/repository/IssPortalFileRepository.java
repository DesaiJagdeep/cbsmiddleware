...
public interface IssPortalFileRepository extends JpaRepository<IssPortalFile, Long>, JpaSpecificationExecutor<IssPortalFile> {
   ...
   @Query("select count(*) from IssPortalFile issPortalFile where issPortalFile.schemeWiseBranchCode =:schemeWiseBranchCode AND issPortalFile.financialYear =:financialYear AND issPortalFile.appSubmitedToKccCount>0")
    ...
    //------------------------------------------------------
    @Query(value = "select count(id) from iss_file_parser where iss_portal_file_id IN (select id from iss_portal_file where financial_year=:financialYear)", nativeQuery = true)
    ...
    @Query(value = "select distinct(scheme_wise_branch_code),branch_name from iss_portal_file where financial_year=:financialYear and scheme_wise_branch_code in (select scheme_wise_branch_code from bank_branch_master where taluka_master_id =:talukaId)", nativeQuery = true)
    ...
    @Query(value = "select pacs_code,pacs_name from iss_portal_file where scheme_wise_branch_code=:schemeBranchCode and financial_year=:financialYear", nativeQuery = true)
