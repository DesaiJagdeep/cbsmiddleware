...
public interface KamalCropRepository extends JpaRepository<KamalCrop, Long>, JpaSpecificationExecutor<KamalCrop> {
   @Query(value = " select * from kamal_crop where farmer_type_master_id=3 and season_master_id=5 and financial_year=:financialYear and pacs_number=:pacsNumber and km_date=:kmDate ",nativeQuery = true)
    ...
    @Query(value = " select * from kamal_crop where pacs_number=:pacsNumber and financial_year=:financialYear and km_date=:kmDate and farmer_type_master_id=3 and (season_master_id=1 or season_master_id=7 ) ",nativeQuery = true)
    ...
    @Query(value = " select * from kamal_crop where farmer_type_master_id=1  and financial_year=:financialYear and pacs_number=:pacsNumber and km_date=:kmDate and (season_master_id=1 or season_master_id=7 ) ",nativeQuery = true)
    ...
    @Query(value = " select * from kamal_crop where farmer_type_master_id=2  and financial_year=:financialYear and pacs_number=:pacsNumber and km_date=:kmDate and (season_master_id=1 or season_master_id=7 ) ",nativeQuery = true)
